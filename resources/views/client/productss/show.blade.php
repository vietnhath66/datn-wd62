...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (!colorSelectElement || !sizeSelect || !stockInfo || !quantityInput || !quantityUpButton || !quantityDownButton) {
            console.log('Không tìm thấy phần tử cần thiết');
            return;
        }

        function updateQuantityInputState() {
            const isDisabled = !colorSelectElement.value.trim() || !sizeSelect.value.trim() || currentStock === 0;
        ...
        colorSelectElement.addEventListener('change', function() {
            ...
            if (selectedColor) {
                ...
                const availableVariantsForColor = variants.filter(variant => variant.name_variant_color.trim() === selectedColor);
                console.log('Available variants for color:', availableVariantsForColor);

                // Get unique sizes for the selected color
                const uniqueSizes = [...new Set(availableVariantsForColor.map(variant => variant.name_variant_size))];
                ...
                if (uniqueSizes.length > 0) {
                    ...
                    uniqueSizes.forEach(size => {
                        ...
                        const hasStock = availableVariantsForColor.some(
                            variant => variant.name_variant_size === size && variant.quantity > 0
                        );

                        const option = document.createElement('option');
                        option.value = size;
                        option.textContent = size + (hasStock ? '' : ' (Hết hàng)');
                        option.disabled = !hasStock; // Disable if no stock for this color and size
                        sizeSelect.appendChild(option);
                    });
                    sizeSelect.disabled = false;
                }
                ...
                const totalQuantityForColor = availableVariantsForColor.reduce((sum, variant) => sum + variant.quantity, 0);
                stockInfo.textContent = totalQuantityForColor > 0 ? `Tổng số lượng còn lại cho màu ${selectedColor}: ${totalQuantityForColor}` : `Màu ${selectedColor}: Hết hàng`;
        ...
        sizeSelect.addEventListener('change', function() {
            ...
            if (selectedColor && selectedSize) {
                ...
                if (matchedVariant) {
                    ...
                    stockInfo.textContent = currentStock > 0 ? `Số lượng còn lại: ${currentStock}` : `Hết hàng`;
                    variantIdInput.value = matchedVariant.id;
                ...
                const totalQuantityForColor = variants.filter(variant => variant.name_variant_color.trim() === selectedColor)
                    .reduce((sum, variant) => sum + variant.quantity, 0);
                currentStock = totalQuantityForColor;
                stockInfo.textContent = currentStock > 0 ? `Tổng số lượng còn lại cho màu ${selectedColor}: ${currentStock}` : `Màu ${selectedColor}: Hết hàng`;
                document.getElementById('selected-product-variant-id').value = '';
            } else {
                currentStock = 0;
                stockInfo.textContent = '';
        ...
        quantityInput.addEventListener('input', function() {
            ...
            console.log('quantityInput - Input value:', this.value, 'Parsed value:', value, 'currentStock:', currentStock);
        ...
        quantityUpButton.addEventListener('click', function() {
            console.log('quantityUpButton clicked - current quantity:', quantityInput.value, 'currentStock:', currentStock);
            if (!quantityInput.disabled && currentStock > 0 && parseInt(quantityInput.value) < currentStock) {
